...
public class EyeManager {
    public static final String TAG = "EyeManager";
    ...
    public EyeManager(Context activity) {
//        super(bs);
        service = activity;
...
//    }
    ...
    public void setup(boolean systemWide) {
        if (isSetup)
            return;
        if (systemWide) {
...
class EyeGestureDetector implements EyeEventReceiver.BaseListener {
    ...
    public void onEyeGesture(EyeGesture gesture) {
        //FIXME: give this to the service?
        Log.d(EyeManager.TAG, "got an eye gesture" + gesture);
